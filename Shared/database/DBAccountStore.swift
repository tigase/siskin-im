...
extension Query {
    ...
    static let accountInsert = Query("INSERT INTO accounts (uuid, name, enabled, server_endpoint, roster_version, status_message, push, additional) VALUES (:uuid, :name, :enabled, :server_endpoint, :roster_version, :status_message, :push, :additional)");
...
public class DBAccountStore {
    ...
    static func create(account: Account) throws {
        try Database.main.writer({ writer in
            try writer.insert(query: .accountInsert, params: ["uuid": account.uuid.uuidString, "name": account.name, "enabled": account.enabled, "server_endpoint": account.serverEndpoint, "roster_version": account.rosterVersion, "status_message": account.statusMessage, "push": account.push, "additional": account.additional])
        })
    }
    ...
    static func list() throws -> [Account] {
        return try Database.main.reader({ reader in
            try reader.select(query: .accountsList, params: [:]).map({ cursor in
