...
final public class DatabaseMigrator: DatabaseSchemaMigrator {
    
    public let expectedVersion: Int = 21;
    ...
    public func upgrade(database: DatabaseWriter, newVersion version: Int) throws {
        ...
        case 14:
            ...
            let itemIds = Set(groupMapping.map { $0.0 });
            try itemIds.forEach({ itemId in
                let groups = groupMapping.filter({ $0.0 == itemId }).map({ $0.1 });
                let annnotations: [RosterItemAnnotation] = try database.select("SELECT annotations FROM roster_items ri WHERE ri.id = :id", cached: false, params: ["id": itemId]).first?.object(for: "annotations") ?? [];
...
""");
        case 16:
            let itemsToUpdate = try database.select("select id, data from chat_history where item_type = \(ItemType.message.rawValue) and data like 'geo:%,%' ", cached: false, params: [:]).map({ ($0.int(for: "id"), $0.string(for: "data")) }).compactMap({ item -> Int? in
        ...
        case 17:
            let idsToRemove = try database.select("select c1.id from chats c1 join (select account, jid, max(timestamp) as timestamp, count(id) from chats group by account, jid having count(id) > 1) c2 on c1.account = c2.account and c1.jid = c2.jid and c1.timestamp < c2.timestamp", cached: false).map({ $0.int(for: "id") });
...
""");
        ...
        case 21:
            let accountsWithoutUUID = try database.select("select name from accounts where uuid is null").map({ $0.string(for: "name") });
