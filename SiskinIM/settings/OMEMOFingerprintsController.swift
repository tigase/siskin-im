...
class OMEMOFingerprintsController: UITableViewController {
    ...
    override func viewWillAppear(_ animated: Bool) {
        ...
        let allIdentities = DBOMEMOStore.instance.identities(forAccount: account, andName: account.description);
        let localDeviceId = Int32(bitPattern: AccountManager.account(for: account)?.omemoDeviceId ?? 0);
