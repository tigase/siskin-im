...
class InvitationChatTableViewCell: BaseChatTableViewCell {
    ...
    func set(item: ConversationEntry, message: String?, appendix: ChatInvitationAppendix) {
        ...
        self.messageField.text = message ?? String.localizedStringWithFormat(NSLocalizedString("Invitation to channel %@", comment: "conversation log invitation to channel label"), appendix.channel.description);
    }
