...
open class DBRosterStore: RosterStore, @unchecked Sendable {
    ...
    public func deleteItem(for context: Context, jid: JID) {
        ...
        ContactManager.instance.update(name: nil, for: .init(account: context.userBareJid, jid: jid.bareJid, type: .buddy))
    }
    ...
    public func initialize(context: Context) {
        return queue.async {
            ...
            let items = try! Database.main.reader({ database in
                try database.select(query: .rosterFindItemsForAccount, params: ["account": context.userBareJid.description]).compactMap({ RosterItem.from(cursor: $0, context: context) })
            });
