...
class ChatViewDataSource {
    ...
    func add(items newItems: [ChatViewItemProtocol], markUnread: Bool = false, loadFinished: Bool = false,  completionHandler: ((Int?)->Void)? = nil) {
        queue.async {
            let start = Date();
...
//                    }
    ...
    func loadItems(before: Int? = nil, limit: Int, unread: Int = 0, completionHandler: ((Int?)->Void)? = nil) {
        guard let account = delegate?.account, let jid = delegate?.jid else {
            return;
        }
        ...
        self.queue.async {
            ...
            DBChatHistoryStore.instance.history(for: account, jid: jid, before: before, limit: limit) { dbItems in
                ...
                self.add(items: dbItems, markUnread: unread > 0, loadFinished: true, completionHandler: { (firstUnread) in
                    completionHandler?(firstUnread);
                });
            }
        }
