...
public class RosterProviderAbstract<Item: RosterProviderItem>: @unchecked Sendable {
        
    private let queue = DispatchQueue(label: "RosterProviderDispatcher");
    ...
    init(controller: AbstractRosterViewController) {
        ...
        DBRosterStore.instance.$items.combineLatest(Settings.$rosterAvailableOnly, PresenceStore.instance.$bestPresences, Settings.$rosterDisplayHiddenGroup).throttle(for: 0.1, scheduler: queue, latest: true).sink(receiveValue: { @Sendable [weak self] items, available, presences, displayHidden in
            self?.updateItems(items: Array(items), presences: presences, available: available, displayHidden: displayHidden);
        }).store(in: &cancellables);
        self.$allItems.drop(while: { @Sendable in $0.isEmpty }).combineLatest(self.$queryString).map({ @Sendable items, query in
            if let query = query, !query.isEmpty {
                return items.filter({ $0.displayName.lowercased().contains(query) || $0.jid.description.lowercased().contains(query) });
            } else {
                return items;
            }
        }).combineLatest(Settings.$rosterItemsOrder).receive(on: self.queue).sink(receiveValue: { @Sendable [weak self] (items, order) in
...
public protocol RosterProviderItem: AnyObject, Sendable {
